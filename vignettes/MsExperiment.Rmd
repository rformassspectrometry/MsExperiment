...
We will also use the `Spectra` package to import mass spectrometry
data and thus load it here too.
...
experimentFiles(lcmse) <- MsExperimentFiles(
    mzML_files = fls,
...
```{r}
lcmse <- linkSampleData(lcmse, with = "experimentFiles.mzML_file",
...
`experimentFiles(lcmse)$mzML_file`. The indices of the rows (samples) in
...
file `"internal_standards.txt"` in `experimentFiles(lcmse)$standards`:

```{r}
lcmse <- linkSampleData(lcmse, with = "experimentFiles.annotations",
...
and the one value in `experimentFiles(lcmse)$annotations`.
...
```{r}
sampleData(lcmse)$raw_file <- normalizePath(fls)
lcmse <- linkSampleData(
    lcmse, with = "sampleData.raw_file = spectra.dataOrigin")
```

The link was thus established between matching values in
`sampleData(lcmse)$raw_file` and `spectra(lcmse)$dataOrigin`.

```{r}
sampleData(lcmse)$raw_file
head(spectra(lcmse)$dataOrigin)
```
...
```{r}
sampleData(lcmse)$sample_id
lcmse@assay$sample

lcmse <- linkSampleData(lcmse, with = "sampleData.sample_id = assay.sample")
lcmse
```
...
```{r}
metadata(lcmse)$other <- data.frame(sample_name = c("study_1", "POOL", "study_2"),
                                  index = 1:3)
b <- lcmse[, 2]
metadata(b)
```
...
```{r}
lcmse <- linkSampleData(lcmse, with = "metadata.other",
...
```{r}
experimentFiles(lcmse)$mzML_file
```
...
```{r}
experimentFiles(lcmse)$annotations
```
