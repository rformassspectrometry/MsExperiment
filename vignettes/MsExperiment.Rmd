...
against the `homo_sapiens.fasta` file. To do so, we will use a search
engine such as MSGF+, that is run using the command line and generates
mzid files. Here, for the sake of time and portability, we will not
actually run MSGF+, but a simple shell script that will generate mzid
files in a temporary R directory.


```{r}
outputs <- sub("mzML", "mzid", basename(experimentFiles(msexp)[["mzmls"]]))
(outputs <- file.path(tempdir(), outputs))
cmd <- paste("touch", outputs)
cmd
```
...
```{r}
(mzids <- dir(tempdir(), pattern = "mzid$", full.names = TRUE))
